...
import java.io.*;
import java.util.*;
//https://github.com/sherifabdlnaby/Lossless-Text-Compression-App/blob/master/src/Huffman.java
/**
 * A class entropy.Huffman compress and decompress for a txt
 */
public class Huffman {
    private static void AddEncodeDFS(HuffmanNode node, String encode, HashMap<Character, String> FinalTable)
    {
        node.Code = encode;
        if(node.Char != '\0')
    ...
    private static HashMap<String,Character> HuffmanEncoding(String inputString, BitSet bitVector) {
        ...
        PriorityQueue<HuffmanNode> frequencyQueue = new PriorityQueue<>(Comparator.comparing(HuffmanNode::getFreq));
        for (Map.Entry<Character, Integer> entry : frequencyTable.entrySet()) {
            frequencyQueue.add(new HuffmanNode(entry.getKey(),entry.getValue()));
        }

        //Creates The Tree
        while (frequencyQueue.size() > 1)
        {
            HuffmanNode left = frequencyQueue.poll();
            HuffmanNode right = frequencyQueue.poll();
            HuffmanNode newNode = new HuffmanNode(left.getFreq() + right.getFreq());
        ...
        HuffmanNode Trie = frequencyQueue.poll();
    ...
    public static String Compress(String inputString, String fileName, String filePath) {
        ...
        try {
            ...
            //Output Files Names
            outputFileName = fileName.substring(0,fileName.lastIndexOf('.'))+".img";
...
//            return -1;
    ...
    public static String Decompress(String fileName, String filePath) {
        ...
        try {
            ...
            //Create output File
            outputFileName = fileName.substring(0,fileName.lastIndexOf('.'))+"Decompressed.rl";
            String outputPath = filePath + "/" + outputFileName;
